...
function createCategoryNav() {
  ...
  nav.innerHTML = genres.map(genre => `
    <a href="#" 
       class="flex min-w-[84px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-10 px-6 text-sm font-bold leading-normal tracking-[0.015em] transition-all duration-300 ${genre.value === currentGenre ? 'bg-primary text-background-dark' : 'text-white/70 hover:text-white hover:bg-white/10'}"
...
function createShowCard(title, show) {
  ...
  const imageUrl = show.image || 'https://via.placeholder.com/400x225/1E1E1E/D4AF37?text=' + encodeURIComponent(title);
